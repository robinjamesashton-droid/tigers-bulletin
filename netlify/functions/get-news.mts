...
function buildPrompt(): string {
  ...
  return "Today's date is " + todayStr + ". Use EXACTLY ONE web search query, no more. The query must be exactly: Hull City AFC news site:hulldailymail.co.uk OR site:skysports.com OR site:bbc.co.uk OR site:wearehullcity.co.uk. From the results of that single search, pick up to 5 distinct stories about Hull City AFC, including at least one from Hull Live/Hull Daily Mail if one appears in the results. If a story is clearly from the 1904 Podcast (the Hull City fan podcast) and appears in results, you may include it too and set \"source\" to \"1904 Podcast\". ONLY include stories published within the last 7 days (on or after " + new Date(today.getTime() - 7 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10) + "). It is fine to return fewer than 5 stories if that's all that qualifies, but return whatever genuinely relevant, recent stories the search found - do not return an empty list if the search actually found usable results. For the \"url\" field, you MUST use the exact, specific article URL returned in your search results for that story - never a homepage, section front page, or a guessed/shortened URL. If you cannot find the specific article URL for a story, exclude that story rather than substituting a homepage link. Respond with ONLY a raw JSON array (no markdown fences, no commentary) where each item has exactly these keys: \"headline\" (string, in your own words), \"source\" (string, publication name), \"date\" (string, human-readable, e.g. '12 Jul 2026'), \"iso_date\" (string, YYYY-MM-DD, best estimate if unstated), \"summary\" (string, 1-2 sentences in your own words, never a direct quote), \"url\" (string, the exact direct article URL from search results). Order the array with the newest story first. Return nothing except that JSON array.";
}
